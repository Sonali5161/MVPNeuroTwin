...
const navItems: { id: PanelId; icon: React.ElementType; label: string }[] = [
  ...
  { id: 'patient-management', icon: Users, label: 'Patient Mgmt' },
  ...
  const handlePanelSwitch = (panelId: PanelId) => {
    ...
    if (user) {
      addAuditLog({
        ...
        action: 'VIEW', resource: `Panel: ${panelId.replace(/-/g, ' ')}`,
        ip: '192.168.1.100', status: 'success', details: `Navigated to ${panelId.replace(/-/g, ' ')} panel`,
      });
    }
  };
  ...
  return (
    <aside className="w-16 lg:w-64 flex-shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 py-2 px-2 space-y-1 overflow-y-auto">
              ...
              <TooltipTrigger asChild>
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? 'bg-neuro/15 text-neuro' : 'text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent'
                  )}
                >
                  <Icon className={cn('w-5 h-5 flex-shrink-0 transition-colors', isActive ? 'text-neuro' : 'group-hover:text-neuro')} />
                  <span className="hidden lg:block truncate">{item.label}</span>
                  {isActive && <div className="hidden lg:block ml-auto w-1.5 h-1.5 rounded-full bg-neuro animate-brain-pulse" />}
                </button>
              </TooltipTrigger>
        ...
        {userRole !== 'researcher' &&
          navItems.filter((item) => !PANEL_PERMISSIONS[item.id]?.includes(userRole as 'researcher' | 'clinician' | 'patient')).map((item) => {
            const Icon = item.icon;
            return (
              <Tooltip key={item.id} delayDuration={0}>
                <TooltipTrigger asChild>
                ...
                <TooltipContent side="right" className="lg:hidden">{item.label} — Restricted</TooltipContent>
              </Tooltip>
            );
          })}
      </nav>
      ...
      <div className="p-2 lg:p-3 border-t border-sidebar-border">
        <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
          <DropdownMenuTrigger asChild>
            <button className="w-full flex items-center gap-2.5 p-2 rounded-lg hover:bg-sidebar-accent transition-colors">
              <div className="w-8 h-8 rounded-full bg-neuro/20 flex items-center justify-center flex-shrink-0">
                <span className="text-neuro text-xs font-bold">{user?.name?.split(' ').map((n) => n[0]).join('') || '??'}</span>
              </div>
              <div className="hidden lg:block flex-1 min-w-0 text-left">
                ...
                <Badge variant="outline" className={cn('text-[9px] px-1.5 py-0 h-4 mt-0.5', roleBadge.color)}>
                  <RoleIcon className="w-2.5 h-2.5 mr-0.5" />{roleBadge.label}
                </Badge>
              </div>
          ...
          <DropdownMenuContent side="top" align="start" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 text-xs" onClick={() => handlePanelSwitch('audit-log')}><Shield className="w-3.5 h-3.5" />Security &amp; Audit Log</DropdownMenuItem>
            <DropdownMenuItem className="gap-2 text-xs" disabled><Settings className="w-3.5 h-3.5" />Account Settings</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 text-xs text-destructive focus:text-destructive" onClick={handleLogout}><LogOut className="w-3.5 h-3.5" />Sign Out</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      ...
      <div className="hidden lg:block px-4 pb-3">
        <div className="text-[10px] text-muted-foreground space-y-1">
          <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-risk-low" /><span>System Online</span></div>
